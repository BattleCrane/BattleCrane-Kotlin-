...
class BOnLevelChangedNode(context: BGameContext) : BNode(context) {
    ...
    val eventHandlerFuncMap = mutableMapOf<Class<*>, (Long, Int) -> Boolean>(
        BOnLevelChangedPipe.IncreasedEvent::class.java to this.increaseLevelFunc,
        BOnLevelChangedPipe.DecreasedEvent::class.java to this.decreaseLevelFunc,
        BOnLevelChangedPipe.ChangedEvent::class.java to this.changeLevelFunc
    )

    override fun handle(event: BEvent): BEvent? {
        val handlerFunc = this.eventHandlerFuncMap[event::class.java]
        if (handlerFunc != null && event is BOnLevelChangedPipe.ActionEvent) {
