...
class BHumanGenerator(context: BGameContext, playerId: Long, x : Int, y : Int) :
    ...
    val onBuildNodeId: Long

    val onBuildPipeId: Long
    ...
    init {
        ...
        //On train marine:
        val onBuildNode = OnBuildNode(this.unitId, context)
        val onBuildPipe = onBuildNode.wrapInPipe()
        this.onBuildNodeId = onBuildNode.id
        this.onBuildPipeId = onBuildPipe.id
        pipeline.bindPipeToNode(BProducableNode.NAME, onBuildPipe)

        //On destroy:
        val onDestroyNode = BHumanFactory.OnDestroyNode(context, this.unitId)
        ...
        //On level action:
        val onLevelActionNode = BHumanFactory.OnLevelActionNode(context, this.unitId)
        ...
        val onHitPointsActionNode = BHumanFactory.OnHitPointsActionNode(context, this.unitId)
    ...
    class OnCreateNode(context: BGameContext, private val playerId: Long) : BNode(context) {
        ...
        private fun isCreatingConditionsPerformed(startX: Int, startY: Int): Boolean {
            val endX = startX + BHumanFactory.WIDTH
            val endY = startY + BHumanFactory.HEIGHT
    ...
    class OnDestroyNode(context: BGameContext, unitId: Long) : BNode(context) {
        ...
        private fun unbindNodes() {
            ...
            this.pipeline.unbindPipeFromNode(BProducableNode.NAME, this.factory.onProduceActionPipeId)
