...
class BHumanTrainTankLvl1(gameContext: BGameContext, owner : BPlayer) : BHumanAction(gameContext, owner), BTargetable {
    ...
    class Producer(context: BGameContext, owner: BPlayer) : BAction.Producer(context, owner) {
        ...
        override fun produceToStackByAbility(stack: MutableSet<BAction>, abilityCount: Int) {
            if (abilityCount > 0) {
                stack.add(BHumanTrainTankLvl1(context, owner))
            }
        }
    }
}